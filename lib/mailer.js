...
export async function sendPlacementEmail({ to, role, studentName, assessorName, date, time, meetLink }) {
  ...
  await transporter.sendMail({
    ...
    to,
    subject: isAssessor
      ? `New Placement Assessment — ${studentName}`
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a1b22;padding:40px 20px">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#10222b;border:1px solid rgba(201,147,44,.22);border-radius:16px;overflow:hidden">
        ...
        <tr>
          <td style="padding:36px 40px 0;text-align:center">
            <div style="display:inline-flex;align-items:center;justify-content:center;width:64px;height:64px;border-radius:18px;background:rgba(16,185,129,.1);border:1.5px solid rgba(16,185,129,.28);margin-bottom:20px">
              <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
            </div>
            <div style="display:inline-block;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.25);border-radius:100px;padding:5px 16px;margin-bottom:16px">
              <span style="font-size:10px;font-weight:700;letter-spacing:.16em;color:#10b981">SESSION CONFIRMED</span>
            </div>
            <h1 style="margin:0 0 10px;font-size:22px;font-weight:800;color:#ffffff;line-height:1.3">${isAssessor ? 'New Placement Session Booked' : 'Your Assessment is Confirmed!'}</h1>
            <p style="margin:0;font-size:14px;color:rgba(255,255,255,.42);line-height:1.65">${isAssessor ? 'A student has booked a session with you' : 'Your placement assessment has been scheduled'}</p>
          </td>
        </tr>
        <tr>
            ...
            <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.09);border-radius:12px;padding:24px">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td style="padding-bottom:14px;width:50%">
                    <div style="font-size:10px;font-weight:700;letter-spacing:.1em;color:rgba(201,147,44,.6);text-transform:uppercase;margin-bottom:4px">Assessor</div>
        ...
        <tr>
          <td style="padding:0 40px 28px;text-align:center">
            <a href="${meetLink}" style="display:inline-block;padding:13px 32px;background:#1a73e8;color:#fff;text-decoration:none;border-radius:10px;font-size:14px;font-weight:700;letter-spacing:.04em">Join Google Meet</a>
            <div style="margin-top:10px;font-size:11px;color:rgba(255,255,255,.25)">${meetLink}</div>
          </td>
        </tr>` : ''}
        <tr><td style="padding:0 40px"><div style="height:1px;background:linear-gradient(to right,transparent,rgba(201,147,44,.18),transparent)"></div></td></tr>
        <tr>
...
export async function sendAssessorWelcomeEmail({ to, name, username, password }) {
  ...
  await transporter.sendMail({
    from,
    to,
    subject: 'Welcome to Grace Academy — Your Assessor Account is Ready',
    html: `
<!DOCTYPE html>
<html lang="en">
<head>
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,.08)">
        ...
        <tr>
          <td style="padding:32px 48px 0;text-align:center">
            ...
            <p style="margin:0;font-size:15px;color:#6b7280;line-height:1.7;max-width:400px;display:inline-block">
              Your assessor account at Grace Academy has been created. Below are your login credentials — keep them safe and change your password on first login.
            </p>
          </td>
        </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding:18px 24px;border-bottom:1px solid #f3f4f6">
                    <div style="font-size:10px;font-weight:700;letter-spacing:.12em;color:#9ca3af;text-transform:uppercase;margin-bottom:5px">Email Address</div>
                    <div style="font-size:15px;font-weight:700;color:#111827;font-family:'Courier New',Courier,monospace">${to}</div>
                  </td>
                </tr>
                <tr>
                  <td style="padding:18px 24px;border-bottom:1px solid #f3f4f6">
                    <div style="font-size:10px;font-weight:700;letter-spacing:.12em;color:#9ca3af;text-transform:uppercase;margin-bottom:5px">Username</div>
                    <div style="font-size:15px;font-weight:700;color:#111827;font-family:'Courier New',Courier,monospace">${username}</div>
                  </td>
                </tr>
                <tr>
                  <td style="padding:18px 24px">
                    <div style="font-size:10px;font-weight:700;letter-spacing:.12em;color:#9ca3af;text-transform:uppercase;margin-bottom:5px">Temporary Password</div>
                    <div style="font-size:15px;font-weight:700;color:#c9932c;font-family:'Courier New',Courier,monospace;letter-spacing:.08em">${password}</div>
                    <div style="font-size:11px;color:#9ca3af;margin-top:6px">You will be asked to change this on first login.</div>
                  </td>
                </tr>
              </table>
            </div>
        ...
        <tr>
          <td style="padding:20px 48px 28px;border-top:1px solid #f3f4f6;text-align:center;background:#fafafa">
            <p style="margin:0 0 6px;font-size:11px;color:#9ca3af;letter-spacing:.08em">GRACE ACADEMY · ASSESSOR PORTAL</p>
