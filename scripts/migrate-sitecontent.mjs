 ...
 * into the proper relational tables.
 *
 * Usage:
 *   $env:DATABASE_URL="postgresql://..." ; node scripts/migrate-sitecontent.mjs
 */
...
async function migrateSlotRequests() {
  ...
  for (const r of row.data) {
    ...
    const existing  = await prisma.slotRequest.findFirst({ where: { assessorId: r.assessorId, createdAt } })
...
async function migrateNotifications() {
  ...
  for (const n of row.data) {
    ...
    const existing  = await prisma.notification.findFirst({
      where: { recipientId: n.recipientId ?? null, type: n.type ?? 'info', createdAt },
    })
    if (existing) { skipped++; continue }
