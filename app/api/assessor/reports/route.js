...
export async function POST(req) {
  ...
  const { bookingId, feedback, feedbackAr, englishLevel, suggestedCourse, clientNow, clientSessionStart, clientMidnight } = await req.json()
  ...
  // All timestamps are from the client browser — avoids server-timezone mismatch entirely.
  // clientSessionStart and clientMidnight are computed in PendingReports.js using new Date()
  // in the browser's local timezone, so "6:00 PM" means 6:00 PM wherever the assessor is.
  const now           = typeof clientNow          === 'number' && clientNow > 0          ? clientNow          : Date.now()
  const sessionStart  = typeof clientSessionStart === 'number' && clientSessionStart > 0 ? clientSessionStart : 0
  const midnight      = typeof clientMidnight     === 'number' && clientMidnight > 0     ? clientMidnight     : Infinity
  if (sessionStart > 0 && now < sessionStart) return NextResponse.json({ error: 'The session has not started yet' }, { status: 400 })
  if (midnight < Infinity && now >= midnight)  return NextResponse.json({ error: 'This is too late to write the report, you should have done that earlier' }, { status: 400 })
