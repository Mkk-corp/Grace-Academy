...
/* ─── sub-components ──────────────────────────────────────────────── */
function StatusBadge({ status, isDark, large }) {
  const cfg = {
    ...
    <>
      ...
      <div className="admin-header">
        <div>
          ...
          <h1 style={{ marginBottom: 6 }}>Session Details</h1>
          <p style={{ fontSize: '.84rem', color: 'var(--text-60)' }}>
            {formatDate(booking.date)}
          </p>
        </div>
        <StatusBadge status={status} isDark={isDark} large />
      </div>
      ...
      <div style={{
        ...
        {[
          ...
          },
          {
            icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="16" height="16"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
            label: 'Weekday',
            value: weekdayFromDate(booking.date),
          },
          {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div style={CARD}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
            ...
            <InfoRow label="Education Level"  value={s.educationLevel} />
            <InfoRow label="English Level"    value={s.englishLevel} />
            <InfoRow label="Prior Courses"    value={s.coursesTaken} />
            <InfoRow label="Expected Level"   value={s.expectedLevel} />
            {s.isEmployed !== null && s.isEmployed !== undefined && (
              <InfoRow label="Employed" value={s.isEmployed ? 'Yes' : 'No'} />
            )}
            {s.jobTitle    && <InfoRow label="Job Title"  value={s.jobTitle} />}
            {s.employer    && <InfoRow label="Employer"   value={s.employer} />}
            {s.faculty     && <InfoRow label="Faculty"    value={s.faculty} />}
            {s.university  && <InfoRow label="University" value={s.university} />}
          </div>
        ...
        <div style={CARD}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
            {a.country && <InfoRow label="Country" value={a.country} />}
            {a.city    && <InfoRow label="City"    value={a.city} />}
            {a.phone   && <InfoRow label="Phone"   value={a.phone} />}
          </div>
      ...
      <div style={{ ...CARD, marginBottom: 24 }}>
        <SectionHeader
          ...
          title="Session Links"
          subtitle="Meeting room and recording"
        />
      ...
      {report ? (
        <div style={CARD}>
          <SectionHeader
            ...
            subtitle={`Submitted by ${report.assessorName || a.name || 'consultant'} on ${new Date(report.submittedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`}
          />
        ...
        <div style={{
          ...
          <p style={{ fontWeight: 700, color: 'var(--text)', fontSize: '.95rem', marginBottom: 6 }}>No Report Yet</p>
