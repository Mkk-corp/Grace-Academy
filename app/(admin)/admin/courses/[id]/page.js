...
export default function CourseDetailPage() {
  ...
  const taStyle = { ...inputStyle, resize: 'vertical', minHeight: 100, lineHeight: 1.6 }

  function Field({ label, required, optional, children }) {
    return (
      <div style={{ marginBottom: 18 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: '.72rem', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: muted, marginBottom: 7 }}>
          {label}
          {required && <span style={{ color: '#ef4444', fontSize: '.8rem' }}>*</span>}
          {optional && (
            <span style={{ fontSize: '.62rem', fontWeight: 700, padding: '1px 7px', borderRadius: 100, background: 'rgba(139,92,246,.1)', border: '1px solid rgba(139,92,246,.25)', color: PURPLE, textTransform: 'none', letterSpacing: 0 }}>
  ...
  function SectionCard({ title, icon, color = GOLD, children }) {
    return (
      <div style={{ background: surf, border: `1px solid ${border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 20 }}>
        <div style={{
          ...
          background: isDark ? `${color}0d` : `${color}07`,
        }}>
          <div style={{
  ...
  function Toggle({ on, onChange, label, labelAr, color = GREEN }) {
    return (
      <div
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 18px', borderRadius: 11, border: `1px solid ${on ? color + '35' : border}`, background: on ? `${color}08` : bg, cursor: 'pointer', transition: 'all .2s' }}
        onClick={() => onChange(!on)}
      >
        <span style={{ fontSize: '.88rem', fontWeight: 600, color: on ? color : text }}>
          {isAr ? labelAr : label}
        </span>
        <div style={{
          width: 42, height: 24, borderRadius: 100, padding: 3,
          background: on ? color : (isDark ? 'rgba(255,255,255,.12)' : '#d1d5db'),
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 0', color: muted }}>
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: 'adSpin .7s linear infinite', marginRight: 10 }}>
  ...
  const courseTitle = form.nameEn || (isNew ? (isAr ? 'دورة جديدة' : 'New Course') : '')
    ...
    <>
      ...
      <div className="admin-header" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <h1 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {courseTitle || (isNew ? (isAr ? 'دورة جديدة' : 'New Course') : '…')}
            </h1>
            {!isNew && form.nameAr && (
              <div style={{ fontSize: '.78rem', color: 'var(--text-60)', direction: 'rtl', textAlign: 'right' }}>{form.nameAr}</div>
            )}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
        ...
        <div>
          ...
          {/* Course Identity */}
          <SectionCard
            title={isAr ? 'معلومات الدورة' : 'Course Identity'}
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <Field label={isAr ? 'اسم الدورة (إنجليزي)' : 'Course Name (English)'} required>
                <input
                  value={form.nameEn}
              ...
              <Field label={isAr ? 'اسم الدورة (عربي)' : 'Course Name (Arabic)'} optional>
                <input
                  value={form.nameAr}
          ...
          <SectionCard
            ...
            color={BLUE}
          >
            <Field label={isAr ? 'الوصف (إنجليزي)' : 'Description (English)'} required>
              <textarea
                value={form.descEn}
            ...
            <Field label={isAr ? 'الوصف (عربي)' : 'Description (Arabic)'} optional>
              <textarea
                value={form.descAr}
          ...
          {/* Marketing */}
          <SectionCard
            title={isAr ? 'الرسالة التسويقية' : 'Marketing Message'}
            ...
            color={PURPLE}
          >
            <Field label={isAr ? 'الرسالة التسويقية (إنجليزي)' : 'Marketing Message (English)'} required>
              <textarea
            ...
            <Field label={isAr ? 'الرسالة التسويقية (عربي)' : 'Marketing Message (Arabic)'} optional>
              <textarea
        ...
        <div>
          ...
          {/* Schedule */}
          <SectionCard
            title={isAr ? 'الجدول الزمني' : 'Schedule & Duration'}
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <Field label={isAr ? 'المدة (أشهر)' : 'Duration (months)'} optional>
                <input
              ...
              <Field label={isAr ? 'عدد الجلسات' : 'Total Sessions'} required>
                <input
            ...
            {/* Speaking sessions toggle */}
            <div style={{ marginTop: 6 }}>
              <Toggle
                on={form.needsSpeaking}
                onChange={v => set('needsSpeaking', v)}
                label="Needs speaking sessions?"
                labelAr="تتضمن جلسات محادثة؟"
                color={BLUE}
              />
              {form.needsSpeaking && (
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px dashed ${border}` }}>
                  <Field label={isAr ? 'عدد جلسات المحادثة' : 'Number of speaking sessions'} required>
                    <input
                      type="number" min="1" step="1"
          ...
          {/* Library */}
          <SectionCard
            title={isAr ? 'المكتبة والمواد' : 'Library & Materials'}
          ...
          >
            <Toggle
              on={form.needsLibrary}
              onChange={v => set('needsLibrary', v)}
              label="Needs library & materials?"
              labelAr="تتضمن مكتبة ومواد؟"
              color={GREEN}
            />

            {form.needsLibrary && (
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px dashed ${border}` }}>
                <div style={{ fontSize: '.72rem', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: muted, marginBottom: 10 }}>
                  ...
                  {LIBRARY_OPTIONS.map(opt => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span style={{ fontSize: '.85rem', fontWeight: checked ? 600 : 400, color: checked ? GREEN : text }}>
          ...
          {!isNew && (
            <SectionCard
              title={isAr ? 'إحصائيات الدورة' : 'Course Stats'}
            ...
            >
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                {[
                  ...
                  <div key={label} style={{ padding: '14px 16px', borderRadius: 11, border: `1px solid ${border}`, background: bg, textAlign: 'center' }}>
                    <div style={{ fontSize: '1.8rem', fontWeight: 900, color, lineHeight: 1 }}>{value}</div>
                    <div style={{ fontSize: '.7rem', color: muted, marginTop: 5, fontWeight: 500 }}>{label}</div>
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                {(form.teachers || []).length === 0 ? (
                  <div style={{ padding: '12px 14px', borderRadius: 9, border: `1px dashed ${border}`, color: muted, fontSize: '.82rem', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {form.teachers.map((t, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8, border: `1px solid ${border}`, background: bg }}>
                        <div style={{ width: 28, height: 28, borderRadius: '50%', background: `${BLUE}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <svg viewBox="0 0 24 24" fill="none" stroke={BLUE} strokeWidth="2" width="13" height="13">
                            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
                          </svg>
                        </div>
                        <span style={{ fontSize: '.84rem', color: text }}>{t.name || t}</span>
                      </div>
                    ))}
                  </div>
                )}
                <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 8, background: isDark ? 'rgba(201,147,44,.07)' : 'rgba(201,147,44,.06)', border: '1px solid rgba(201,147,44,.2)', fontSize: '.72rem', color: GOLD }}>
                  {isAr ? 'سيتم تعيين المعلمين تلقائياً من خلال النظام' : 'Teachers are assigned automatically by the system'}
                </div>
              </div>
            </SectionCard>
          )}
      ...
      <div style={{
        ...
        {saved && <span style={{ fontSize: '.83rem', color: GREEN, display: 'flex', alignItems: 'center', gap: 5 }}><svg viewBox="0 0 24 24" fill="none" stroke={GREEN} strokeWidth="2.5" width="14" height="14"><polyline points="20 6 9 17 4 12"/></svg>{isAr ? 'تم الحفظ بنجاح' : 'Changes saved'}</span>}
        <button
          onClick={save}
          disabled={saving}
          style={{
