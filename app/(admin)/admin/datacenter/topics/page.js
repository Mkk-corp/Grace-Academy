...
const S = {
  en: {
    ...
    subtitle: 'Manage topics assessors can select when working with students',
    addBtn: '+ Add Topic',
  ...
  ar: {
    back: 'مركز البيانات',
    title: 'مواضيع المحادثة',
    subtitle: 'إدارة المواضيع التي يختارها المستشارون للحديث عنها مع الطلاب',
    addBtn: '+ إضافة موضوع',
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
        ...
        {!loading && search.trim() && filtered.length === 0 && topics.length > 0 && (
          <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-60)', fontSize: '.88rem' }}>
            {isAr ? 'لا توجد نتائج مطابقة للبحث' : 'No topics match your search'}
          </div>
        )}
      </div>

      {editing && (
        <TopicModal topic={editing} onSave={save} onClose={() => { setEditing(null); setError('') }} s={s} error={error} isAr={isAr} isDark={isDark} />
      )}
...
function TopicModal({ topic, onSave, onClose, s, error, isAr, isDark }) {
